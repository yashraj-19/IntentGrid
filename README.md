...
classification model, explainability, and an interactive dashboard.
...
individual "leads" misses this, since the unit that actually matters is the
*account*. IntentGrid scores each account on two axes and turns that into a
single, explainable, actionable number:
...
| Fit vs Intent heatmap | Classic 2x2 prioritization matrix: Target Now / Nurture / Monitor / Deprioritize |
...
6. **Dashboard (`app.py`)**: 5 tabs, namely Overview, Account Explorer
   (filterable ranked list), Account Heatmap (fit vs intent quadrant matrix),
   Account Deep-Dive (SHAP bar chart for a single account), and Model
   Performance.
...
- Given more time, natural next steps include intent *trends* over time (is
  intent rising or falling this week?), CRM integration (Salesforce/HubSpot)
  to close the loop between predicted score and actual pipeline outcome, and
  a next-best-action recommender suggesting which channel/content to use
  next.
